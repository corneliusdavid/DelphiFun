...
// Version     : 1.0
// Description : My first megademo presented at optimize 2001
//
//------------------------------------------------------------------------
program Biohazard;
...
const
  WND_TITLE = 'Biohazard - Optimize 2001';
...
begin
  ...
  case Stage of
   -1 : ShowLoading;
    ...
    1 : ;                   // Used to init and start Bass sound in main loop
    2 : drawIntro;          // Draws the intro screen
    3 : drawIntroTunnel;    // Intro wallpaper becomes a tunnel
    4 : drawFetus;          // Draws the fetus in cage
    5 : drawFetusInRoom;    // Draws the fetus in cage
    6 : drawTunnel;         // Draws the tunnel and blob in window
    7 : drawBlob;           // Draws the blob
    8 : drawMetaballs;      // Draws the blob
    9 : drawTentacles;      // Draws the tentacles
  end;
end;
...
{$ENDIF}
  ...
  BASS_Start;
  Stream := BASS_StreamCreateFile(FALSE, PChar('music.mp3'), 0, 0, 0);
  if Stream = 0 then
  begin
...
{$ENDIF}
      if Stage = 10 then Finished :=TRUE;
    end;
  end;
  glKillWnd(FALSE);
  Result := msg.wParam;

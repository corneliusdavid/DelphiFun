...
// Date        : 8 April 2001
// Version     : 1.0
// Description : Spherical Environment Mapping demo
//
//------------------------------------------------------------------------
program spherical;
...
const
  WND_TITLE = 'Spherical Environment Mapping by Jan Horn';
...
begin
  ...
  case Shape of
    ...
    2 : begin
          ...
          gluCylinder(MyQuadratic,1.0,1.0,3.0,32,32);	// A Cylinder With A Radius Of 0.5 And A Height Of 2
        end;
    3 : gluDisk(MyQuadratic,0.5,1.5,32,32);				// Draw A Disc (CD Shape) With An Inner Radius Of 0.5, And An Outer Radius Of 2.  Plus A Lot Of Segments ;)
    4 : gluSphere(MyQuadratic,1.3,32,32);				// Draw A Sphere With A Radius Of 1 And 16 Longitude And 16 Latitude Segments
    5 : begin
          glTranslatef(0.0,0.0,-1.5);					// Center The Cone
          gluCylinder(MyQuadratic,1.0,0.0,3.0,32,32);	// A Cone With A Bottom Radius Of .5 And A Height Of 2
        end;
  end;
  glPopMatrix();
...
begin
  ...
  LoadTexture('reflection.bmp', ReflectionTex);
  ...
  xSpeed :=0.2;   // start with some movement
  ySpeed :=0.3;
end;
...
begin
  if (keys[VK_PRIOR]) then z := z - 0.1;
  if (keys[VK_NEXT])  then z := z + 0.1;
  ...
  if (keys[ord('1')]) then Shape :=1;
  if (keys[ord('2')]) then Shape :=2;
  if (keys[ord('3')]) then Shape :=3;
  if (keys[ord('4')]) then Shape :=4;
  if (keys[ord('5')]) then Shape :=5;
...
begin
  ...
  if not glCreateWnd(800, 600, FALSE, 16) then
  begin
    Result := 0;
    Exit;
  end;

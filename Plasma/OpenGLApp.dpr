...
var
  ...
  TextureArray : Array[1..6] of glUint;
  CurrentTexture : Integer;

  // User vaiables
  R, G, B : Array[0..15] of glFloat;
  Background : Boolean;
  Layers : Integer;
...
begin
  ...
  // Draw plasma color effects
  glTranslatef(0.0,0.0,-1);
  glBindTexture(GL_TEXTURE_2D, TextureArray[CurrentTexture]);
  for I :=0 to Layers do
  begin
    Temp :=(ElapsedTime/96 + I*2) / 4;
    XSize :=1.0 + 0.5*cos(Temp);
    YSize :=1.0 + 0.5*cos(0.8*Temp);
    Angle :=180.0 + 180.0*sin((Temp-I)/5);
    XPos :=0.5*sin(Temp/8);
    YPos :=0.4*cos(Temp/4);
    ...
    glPushMatrix();
      ...
      glColor3f(R[I MOD 16], G[I MOD 16], B[I MOD 16]);
      glBegin(GL_QUADS);
...
//  glDepthFunc(GL_LESS);                    // The Type Of Depth Test To Do
  ...
  LoadTexture('background.jpg', BackgroundTex, FALSE);  // Load the Texture
  LoadTexture('texture1.jpg', TextureArray[1], FALSE);  // Load the Texture
  LoadTexture('texture2.jpg', TextureArray[2], FALSE);  // Load the Texture
  LoadTexture('texture3.jpg', TextureArray[3], FALSE);  // Load the Texture
  LoadTexture('texture4.jpg', TextureArray[4], FALSE);  // Load the Texture
  LoadTexture('texture5.jpg', TextureArray[5], FALSE);  // Load the Texture
  LoadTexture('texture6.jpg', TextureArray[6], FALSE);  // Load the Texture

  // cycle through all colors and allow each color get a max value {sin(0 - 2pi)}
  For I :=0 to 15 do
  begin
    R[I] :=0.1 + 0.2*sin(I*pi/8);
    G[I] :=0.1 + 0.2*sin((I+5)*pi/8);
    B[I] :=0.1 + 0.2*sin((I+10)*pi/8);
  end;
end;
    ...
    WM_TIMER :                     // Add code here for all timers to be used.
      begin
        ...
        begin
          ...
          SetWindowText(h_Wnd, PChar(WND_TITLE + '   ' + intToStr(Layers) + ' layers   [' + intToStr(FPSCount) + ' FPS]'));
          FPSCount := 0;
          Result := 0;
        end;
      end;

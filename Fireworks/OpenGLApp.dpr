...
type TParticle = Record
     ...
     TFirework = Record
       Particle : Array[0..NumParticles] of TParticle;     // particles in the explosion
       Trail : Array[0..TrailLength] of TParticle;         // particles in the tail
...
begin
  ...
  // tail particles
  for I :=0 to TrailLength do
    with Firework[N].Trail[I] do
    begin
      X :=0;
      Y :=0;
      Z :=650 + random(50);             // will act as duration for particle
...
begin
  ...
  if not glCreateWnd(GetSystemMetrics(SM_CXSCREEN),
                     GetSystemMetrics(SM_CYSCREEN), FALSE, 32) then
  begin
    Result := 0;
    Exit;
  end;

...
begin
  ...
  LoadTexture('wood1.bmp', WoodTexture);    // Load the Texture
...
begin
  if (keys[VK_UP])    then xAngle := xAngle - 0.5;
  if (keys[VK_DOWN])  then xAngle := xAngle + 0.5;
  if (keys[VK_RIGHT]) then yAngle := yAngle - 0.5;
  if (keys[VK_LEFT])  then yAngle := yAngle + 0.5;
    ...
    WM_TIMER :                     // Add code here for all timers to be used.
      begin
        ...
        begin
          ...
          SetWindowText(h_Wnd, PChar(WND_TITLE + '   [' + intToStr(FPSCount) + ' FPS]   Drops fall every ' + IntTOStr(RainInterval) + 'ms'));
          FPSCount := 0;
          Result := 0;
        end
